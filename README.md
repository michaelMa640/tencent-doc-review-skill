...
> - 推荐做法是：**把这个仓库当成 OpenClaw 工作区来用**；或者先安装项目，再把 [`skills/tencent_doc_review_native`](skills/tencent_doc_review_native) 单独复制到 OpenClaw 的 `skills/` 目录
...
- 保留 Agent 侧事实核查路径语义
- 当前运行时如果 Agent 路径不可用，会自动 fallback 到 API 搜索
- fallback 信息会写进检索痕迹，便于排障
...
```text
tdr-debug-20260330-153012-product-research-report-sample-deepseek.json
```
...
- 因为仓库里已经带了原生 skill：
  - [`skills/tencent_doc_review_native/SKILL.md`](skills/tencent_doc_review_native/SKILL.md)

如果你的 OpenClaw 工作区不是这个仓库，也可以：

- 先安装本项目
- 再把 [`skills/tencent_doc_review_native`](skills/tencent_doc_review_native) 复制或软链接到：
...
- `auto`：保留 Agent 路径语义；当前运行时若 Agent 路径不可用，会自动 fallback 到搜索 API
- `agent`：保留 Agent 侧事实核查路径语义；当前运行时仍待接入独立 Agent 搜索执行器
- `api`：强制使用搜索 API 事实核查
...
这条链路使用的是 [`src/tencent_doc_review/tencent_doc_client.py`](src/tencent_doc_review/tencent_doc_client.py)，代码里会**同时强制检查**：
...
- 使用项目内置的 [`src/tencent_doc_review/access/openclaw_bridge.py`](src/tencent_doc_review/access/openclaw_bridge.py)
...
- [`default_product_research_review_rules.md`](templates/default_product_research_review_rules.md)
- [`default_product_research_structure_template.md`](templates/default_product_research_structure_template.md)
...
然后编辑 [`.env.example`](.env.example) 对应字段，填入你自己的 `.env`。
...
这样可以保留 Agent 路径语义，并在当前运行时于 Agent 路径不可用时回退到 API。
...
- 内置 [`src/tencent_doc_review/access/claude_code_bridge.py`](src/tencent_doc_review/access/claude_code_bridge.py)
- 默认会尝试自动探测：
  - 当前 Python 解释器
  - `claude` CLI
- 自动生成 bridge 启动参数
...
代码会优先读取根目录 [`templates`](templates)。
